...
const uint RATING=13;//计算胜率
const uint FIGHT=14;//对战信息
...
struct User//在线用户
{
    ...
    User toUserObject(const QJsonObject &jsonObj) {
        ...
        user.victory =jsonObj["vectory"].toInt();
        user.defeat =jsonObj["defeat"].toInt();
        return user;
    }
    ...
    QJsonObject toJsonObject() const {
        ...
        jsonObj["port"] = port;
        jsonObj["pwd"] = pwd;
        jsonObj["victory"]=victory;
        jsonObj["defeat"]=defeat;
        return jsonObj;
    }
...
public:
    ...
    bool userHasNoPokemons(QSqlQuery &query, const QString &username);
    int getUseridByUsername(QSqlQuery &query, const QString &username);
    void updateExperience(QSqlQuery &query, int currentpkmid,int exp);
    int getUserIdByUsername(const QString &username);
    void transferPokemonOwnership(QSqlQuery &query, int changepkmid, int newOwnerId);
    void updateGameRecord(QSqlQuery &query, const QString &username, int result);
    QString findOwnerNameByPokemonId(int pokemonId);
    void levelUpPokemon(int currentpkmid);
